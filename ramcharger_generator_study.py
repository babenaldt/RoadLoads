#!/usr/bin/env python3
"""
Ram 1500 Ramcharger Generator Sizing Study
===========================================
Batch simulation of Ram 1500 Ramcharger EREV configurations across multiple drive cycles.
...
# Configuration
# =============================================================================
STUDY_NAME = "ram_ramcharger_generator_study_2026-01-02"
OUTPUT_BASE = "outputs"
...
GENERATOR_CAPACITY_KW = 130.0


# =============================================================================
# Helper Functions
# =============================================================================
def load_ramcharger_presets(json_path: str = 'vehicle_presets.json') -> List[Dict[str, Any]]:
    """Load all Ram 1500 Ramcharger EREV presets from JSON file."""
    with open(json_path, 'r') as f:
        data = json.load(f)
    
    # Filter for Ram Ramcharger EREV variants
    ram_presets = [
        p for p in data['presets'] 
        if 'Ram 1500 Ramcharger' in p.get('name', '')
    ]
...
def generate_efficiency_bar_chart(
    ...
    vehicles.sort(key=lambda x: (0 if x == 'Ram 1500 Ramcharger' else 1, x))
    
    # Create short labels for vehicles
    short_labels = []
    for v in vehicles:
        if v == 'Ram 1500 Ramcharger':
            short_labels.append('Base')
        else:
            short_labels.append(v.replace('Ram 1500 Ramcharger + ', '').replace(' Trailer', ''))
...
def generate_power_demand_chart(
    ...
    axes[0].axhline(y=130, color='red', linestyle='--', linewidth=2, label='Generator (130 kW)')
    ...
    axes[1].axhline(y=130, color='red', linestyle='--', linewidth=2, label='Generator (130 kW)')
...
def generate_soc_profile_charts(
    ...
    for idx, cycle_key in enumerate(cycles):
        ...
        for r in cycle_results:
            if r.vehicle_name == 'Ram 1500 Ramcharger':
                label = 'Base'
                linestyle = '-'
                linewidth = 2.5
            else:
                label = r.vehicle_name.replace('Ram 1500 Ramcharger + ', '').replace(' Trailer', '')
                linestyle = '--'
                linewidth = 1.5
...
def generate_range_comparison_chart(
    ...
    range_results_sorted = sorted(range_results, key=lambda x: (0 if x.vehicle_name == 'Ram 1500 Ramcharger' else 1, x.vehicle_name))
    ...
    for r in range_results_sorted:
        if r.vehicle_name == 'Ram 1500 Ramcharger':
            ...
            short_labels.append(r.vehicle_name.replace('Ram 1500 Ramcharger + ', '').replace(' Trailer', ''))
...
<head>
    ...
    <title>Ram 1500 Ramcharger Generator Sizing Study</title>
    <style>
        body {{
...
<body>
    <h1>🔋 Ram 1500 Ramcharger EREV Generator Sizing Study</h1>
    <p class="timestamp">Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
    
    <div class="summary-box">
        <h3>Study Overview</h3>
        <p>This study evaluates the Ram 1500 Ramcharger EREV's <strong>130 kW generator</strong> across various towing configurations and demanding drive cycles.</p>
        <ul>
...
"""
    ...
    for preset in presets:
        name = preset['name']
        if name == 'Ram 1500 Ramcharger':
            display_name = 'Base (No Trailer)'
        else:
            display_name = name.replace('Ram 1500 Ramcharger + ', '')
...
"""
    ...
    <div class="section">
        <p>Peak and average power demand at the wheels for each vehicle/cycle combination. Values exceeding the 130 kW generator capacity are highlighted.</p>
        <table>
            <tr>
...
"""
    ...
    for vehicle in vehicles:
        if vehicle == 'Ram 1500 Ramcharger':
            display_name = 'Base (No Trailer)'
        else:
            display_name = vehicle.replace('Ram 1500 Ramcharger + ', '')
...
"""
    ...
    for r in range_results_sorted:
        if r.vehicle_name == 'Ram 1500 Ramcharger':
            ...
            display_name = r.vehicle_name.replace('Ram 1500 Ramcharger + ', '')
...
""".format(
    ...
    if deficit_configs:
        html += "            <li><strong>Power Deficits Detected:</strong> The 130 kW generator was insufficient in the following scenarios:</li>\n            <ul>\n"
        for d in deficit_configs:
            name = 'Base' if d['vehicle'] == 'Ram 1500 Ramcharger' else d['vehicle'].replace('Ram 1500 Ramcharger + ', '')
            html += f"                <li>{name} on {d['cycle']}: {d['count']} deficit events, max shortfall of {d['max_deficit']:.1f} kW</li>\n"
        html += "            </ul>\n"
    else:
        html += "            <li><strong>No Power Deficits:</strong> The 130 kW generator was sufficient for all tested configurations and cycles.</li>\n"
    ...
    peak_name = 'Base' if max_peak.vehicle_name == 'Ram 1500 Ramcharger' else max_peak.vehicle_name.replace('Ram 1500 Ramcharger + ', '')
    avg_name = 'Base' if max_avg.vehicle_name == 'Ram 1500 Ramcharger' else max_avg.vehicle_name.replace('Ram 1500 Ramcharger + ', '')
...
"""
    ...
    # Range impact
    base_range = next((r for r in range_results if r.vehicle_name == 'Ram 1500 Ramcharger'), None)
    if base_range:
        ...
        worst_name = worst_range.vehicle_name.replace('Ram 1500 Ramcharger + ', '')
...
"""
    ...
    <footer style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #ddd; color: #95a5a6; text-align: center;">
        <p>Ram 1500 Ramcharger Generator Sizing Study • Road Load Simulator</p>
    </footer>
</body>
</html>
"""
...
def main():
    ...
    print("=" * 60)
    print("Ram 1500 Ramcharger EREV Generator Sizing Study")
    print("=" * 60)
    ...
    print("\nLoading Ram 1500 Ramcharger EREV presets...")
    presets = load_ramcharger_presets()
    ...
    print(f"  - Configs exceeding 130kW at 75mph: {exceed_75mph_count}")
    print(f"  - Configs that cannot sustain 75mph on generator: {cannot_sustain_count}")
    
    if exceed_75mph_count > 0:
        print(f"\n  ⚠️ WARNING: {exceed_75mph_count} configuration(s) require more than 130 kW at 75 mph!")
        for r in range_results_75mph:
            if r.exceeds_generator:
                name = 'Base' if r.vehicle_name == 'Ram 1500 Ramcharger' else r.vehicle_name.replace('Ram 1500 Ramcharger + ', '')
...
def generate_max_speed_chart(
    ...
    results_sorted = sorted(results, key=lambda x: (0 if 'Ram 1500 REV' == x.vehicle_name else 1, x.vehicle_name))
    
    short_labels = []
    for r in results_sorted:
        if r.vehicle_name == 'Ram 1500 REV':
            short_labels.append('Base')
        else:
            short_labels.append(r.vehicle_name.replace('Ram 1500 REV + ', '').replace(' Trailer', ''))
    ...
    ax.set_title('Maximum Steady-State Speed on Generator Power Alone (130 kW)', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(short_labels, rotation=45, ha='right')
    ax.legend(loc='upper right')
    ax.grid(True, alpha=0.3, axis='y')
...
"""
        for r in exceed_configs:
            name = 'Base' if r.vehicle_name == 'Ram 1500 Ramcharger' else r.vehicle_name.replace('Ram 1500 Ramcharger + ', '')
            html += f"            <li><strong>{name}</strong>: Requires {r.peak_power_kw:.1f} kW at 75 mph (exceeds 130 kW generator)</li>\n"
...
"""
    ...
    for preset in presets:
        name = preset['name']
        display_name = 'Base (No Trailer)' if name == 'Ram 1500 Ramcharger' else name.replace('Ram 1500 Ramcharger + ', '')
        html += f"""            <tr>
                <td class="vehicle-name">{display_name}</td>
                <td>{preset['mass']:.0f}</td>
                <td>{preset['mass'] * 2.205:.0f}</td>
...
"""
    ...
    <div class="section">
        <p>Maximum steady-state speed achievable on flat ground using generator power (130 kW) alone, with no battery assist.</p>
        <table>
            <tr>
                <th>Configuration</th>
                <th>Max Speed (mph)</th>
...
"""
    ...
    for r in sorted(generator_speed_results, key=lambda x: (0 if x.vehicle_name == 'Ram 1500 Ramcharger' else 1, x.vehicle_name)):
        display_name = 'Base (No Trailer)' if r.vehicle_name == 'Ram 1500 Ramcharger' else r.vehicle_name.replace('Ram 1500 Ramcharger + ', '')
        sustain_class = 'ok' if r.can_sustain_75mph else 'exceed'
        sustain_text = 'Yes ✓' if r.can_sustain_75mph else 'No ✗'
        power_class = '' if r.power_at_75mph_kw <= 130 else 'exceed'
        html += f"""            <tr>
                <td class="vehicle-name">{display_name}</td>
...
"""
    ...
    <div class="section">
        <p>Peak and average power demand at the wheels. Values exceeding 130 kW are highlighted.</p>
        <table>
            <tr>
                <th rowspan="2">Configuration</th>
"""
...
"""
    ...
    for vehicle in vehicles:
        display_name = 'Base (No Trailer)' if vehicle == 'Ram 1500 Ramcharger' else vehicle.replace('Ram 1500 Ramcharger + ', '')
        html += f'            <tr>\n                <td class="vehicle-name">{display_name}</td>\n'
        ...
        for cycle_key in cycles:
            ...
            if matching:
                r = matching[0]
                peak_class = 'exceed' if r.peak_power_kw > 130 else ''
                avg_class = 'exceed' if r.avg_power_kw > 130 else ''
                html += f'                <td class="{peak_class}">{r.peak_power_kw:.1f}</td>\n'
                html += f'                <td class="{avg_class}">{r.avg_power_kw:.1f}</td>\n'
            else:
                html += '                <td>-</td>\n                <td>-</td>\n'
...
"""
    ...
    for r in sorted(range_results_multi, key=lambda x: (0 if x.vehicle_name == 'Ram 1500 Ramcharger' else 1, x.vehicle_name)):
        display_name = 'Base (No Trailer)' if r.vehicle_name == 'Ram 1500 Ramcharger' else r.vehicle_name.replace('Ram 1500 Ramcharger + ', '')
        html += f"""            <tr>
                <td class="vehicle-name">{display_name}</td>
                <td><strong>{r.total_range_miles:.0f}</strong></td>
...
"""
    ...
    <div class="section">
        <p>Range at constant 75 mph on flat ground starting from 100% SOC. Peak power values exceeding 130 kW generator capacity are highlighted.</p>
        <p><strong>Note:</strong> "Generator-Assisted Range" shows miles traveled while generator is running. When power demand exceeds 130 kW, 
        the battery supplements the generator. All configurations deplete their 27-gallon fuel tank at roughly the same rate because 
        the generator output is capped at 130 kW, but high-drag configurations would be speed-limited to "Max Gen Speed" after battery depletion.</p>
        <table>
            <tr>
                <th>Configuration</th>
                <th>Total Range (mi)</th>
...
"""
    ...
    for r in sorted(range_results_75mph, key=lambda x: (0 if x.vehicle_name == 'Ram 1500 Ramcharger' else 1, x.vehicle_name)):
        display_name = 'Base (No Trailer)' if r.vehicle_name == 'Ram 1500 Ramcharger' else r.vehicle_name.replace('Ram 1500 Ramcharger + ', '')
